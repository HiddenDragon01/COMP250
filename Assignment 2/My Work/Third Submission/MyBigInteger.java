...
public class MyBigInteger  implements Comparable<MyBigInteger> {   //  We will cover what "implements Comparable" means later
	...
	public MyBigInteger dividedBy( MyBigInteger  divisor ) throws Exception{
		...
		if (divisor.getCoefficients().size() == 1 && divisor.getCoefficients().get(0) == 0) {
			System.out.println("ERROR: can't divide by zero");
			throw new Exception();
		}
		
		
		int compare = this.compareTo(divisor);
				
		if (compare < 0) {
			quotient.getCoefficients().add(Integer.valueOf(0));
			return quotient;
		}
		if (compare == 0) {
			quotient.getCoefficients().add(Integer.valueOf(1));
			return quotient;
		}
		
		if (divisor.getCoefficients().size() == this.getCoefficients().size()) {
			for (int i = 1; i < this.base; i++) {
				if ((this.minus(divisor.timesSingleDigit(i))).compareTo(divisor) < 0) {
					quotient.getCoefficients().add(i);
					return quotient;
				}
			}
		} else {
			
			MyBigInteger dividableNumber = new MyBigInteger(this.base);
			for (int i = this.getCoefficients().size() - divisor.getCoefficients().size() ; i < this.getCoefficients().size(); i++) {
				
				dividableNumber.getCoefficients().add(this.getCoefficients().get(i));
			}
			
			if (dividableNumber.compareTo(divisor) < 0) {
				dividableNumber.getCoefficients().add(0,this.getCoefficients().get(this.getCoefficients().size() - divisor.getCoefficients().size() - 1));
			}
			...
			int quo = 1;
			for (int i = 1; i < this.base; i++) {
				if ((dividableNumber.minus(divisor.timesSingleDigit(i))).compareTo(divisor) < 0) {
					quotient.getCoefficients().add(Integer.valueOf(i));
					quo = i;
			...
			if (dividableNumber.getCoefficients().size() == this.getCoefficients().size()) {
				return quotient;
			}
			
			remainder = dividableNumber.minus(divisor.timesSingleDigit(Integer.valueOf(quo)));
			remainder.getCoefficients().add(0,this.getCoefficients().get(this.getCoefficients().size() - dividableNumber.getCoefficients().size() - 1));
			
					
			for (int i = this.getCoefficients().size() - dividableNumber.getCoefficients().size() - 2; i >= 0; i-- ) {
				
				
				
				while (remainder.getCoefficients().get(remainder.getCoefficients().size() - 1) == 0 && remainder.getCoefficients().size() > 1) {
					
					remainder.getCoefficients().remove(remainder.getCoefficients().size() - 1);
					
				}

				if (remainder.compareTo(divisor) < 0)  {
					quotient.getCoefficients().add(0,Integer.valueOf(0));
					remainder.getCoefficients().add(0,this.getCoefficients().get(i));
				
					
					
				} else {
					for (int j = 1; j < this.base; j++) {
						if ((remainder.minus(divisor.timesSingleDigit(j))).compareTo(divisor) < 0) {
							quotient.getCoefficients().add(0,j);
							remainder = remainder.minus(divisor.timesSingleDigit(j));
							remainder.getCoefficients().add(0,this.getCoefficients().get(i));
							break;
						}
	...
	public MyBigInteger convert(int newBase) throws Exception {
    	...
    	if (this.base > newBase) {
    		...
    		MyBigInteger newBaseDivisor = new MyBigInteger(this.base);
    		MyBigInteger mod = new MyBigInteger(this.base);
    		newBaseDivisor.getCoefficients().add(newBase);
    		
    		while (remainder.compareTo(newBaseDivisor) >= 0) {
    			mod = remainder.mod(newBaseDivisor);
    			result.getCoefficients().add(mod.getCoefficients().get(0));
    			remainder = remainder.dividedBy(newBaseDivisor);
    	...
    	if (this.base < newBase) {
    		...
    		MyBigInteger multiplyBig = new MyBigInteger(Integer.valueOf(this.base).toString(),newBase);
        	
        	MyBigInteger multiplyBigClone = multiplyBig.clone();
        	
        	MyBigInteger bigCoefficient = null;
    		
    		for (int i = 0; i < this.getCoefficients().size(); i++) {
    			
    			int coefficient = this.getCoefficients().get(i);
    			bigCoefficient = new MyBigInteger(Integer.valueOf(coefficient).toString(),newBase);
    			
    			
    			if (i == 0) {
    				result.getCoefficients().add(this.getCoefficients().get(0));
    				
    			}
    			    				
    		    if (i == 1) {
    				bigCoefficient = bigCoefficient.times(multiplyBig);
    				result = result.plus(bigCoefficient);
    			} 
    		    
    		    if (i > 1) {
    				multiplyBig = multiplyBig.times(multiplyBigClone);
    				bigCoefficient = bigCoefficient.times(multiplyBig);
    				result = result.plus(bigCoefficient);
    		    }
	...
	public ArrayList<MyBigInteger>  primeFactors() throws Exception {
		...
		MyBigInteger factoredNum = this.clone();		
		MyBigInteger zero = new MyBigInteger("0",this.base);
		MyBigInteger one = new MyBigInteger("1",this.base);
		MyBigInteger two = new MyBigInteger("2",this.base);
		MyBigInteger Potentialfactor = two;
		
		while (true) {
			
			if (Potentialfactor.times(Potentialfactor).compareTo(factoredNum) > 0) {
				if (factoredNum.compareTo(one) > 0)
				...
				while (factoredNum.mod(Potentialfactor).compareTo(zero) == 0) {
				
					factors.add(Potentialfactor);
					factoredNum = factoredNum.dividedBy(Potentialfactor);
					
				}
				
				Potentialfactor = Potentialfactor.plus(one);
			}
